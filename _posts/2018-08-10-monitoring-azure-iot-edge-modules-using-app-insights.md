...
The Azure IoT Edge offering is a great of way of shifting the load from the cloud to the Edge of the network, closer to the source of the data. Azure IoT Edge offering allows us to run code and data pipeline close to where the data is originated to improve performance and improve security and privacy. The Azure IoT Edge provide the flexibility to deploy and run these modules as docker containers. 
...
There is enough documentation on the Microsoft [MSDN](https://docs.microsoft.com/en-us/azure/application-insights/app-insights-create-new-resource) on how to create an App insight resource, so we do not need to cover it here. 
...
Once we have a resource in our Azure subscription, we can then deploy the Azure App Insight docker image either via the Azure IoT Hub or directly via the command line. The end command should look like this: 
...
This will tell docker to pull the docker image `microsoft/applicationinsights` and gives it the parameter `ikey`. This will taka a minute before it starts sending telemetry to our Azure App Instance. 

# Configuring the Reports
Previously, we used to be able to configure our App Insights dashboard to have whatever reports that we wanted. This was recently changed to lock in certain reports only. However, we can still configure our main dashboard to produce whatever reports we want. 

The first thing we will need to do is to `create a new dashboard` in Azure portal. This's to allow us to add the new reports that Azure App Insights comes with out-of-the-box. Once we do that, we can then pull the reports we want. I have pulled three reports in the screenshot below. These are `Docker Overview`, `Servers`, and `Performance Timeline`. 
...
Each of the containers is represented by a line in the `Activity by Container` table. Clicking on any of these containers will take use to a more detailed reports about this particular container. An example of this is shown by: 
...
An other tile that we can add to the dashboard has custom events. By default it will include all `Docker Events`. Things like `start` and `stop` of containers. An example of these reports below: 
